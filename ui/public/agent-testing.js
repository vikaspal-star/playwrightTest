...
window.AgentTesting = (() => {
  ...
  function renderConnection() {
    ...
    const connection = node("section", { class: "surface agent-card" }, node("h3", {}, "Choose how to test"), field("Testing method", draft.testType || "api", v => { draft.testType = v; if (v === "manual") { draft.iterations = 1; draft.scenarios.forEach(s => s.mode = "scripted"); } changed(); render(); }, { choices: [["api", "API testing — call a chat endpoint"], ["manual", "Manual testing — enter observed replies"]] }), node("p", { class: "agent-muted" }, manual ? "Follow each scenario in your agent's interface and enter its actual replies. An API connection is not required." : "Maya sends JSON messages to this endpoint and reads a text reply. Each scenario gets a fresh session ID."));
    connection.append(node("div", { class: "agent-form-grid" }, field("Agent name", draft.name, v => draft.name = v, { maxlength: 100, placeholder: "Customer support agent" }),
      ...
      field("Server headers variable (optional)", draft.headersEnv, v => draft.headersEnv = v, { placeholder: "MAYA_AGENT_SUPPORT_HEADERS", help: "Name of a server environment variable containing JSON headers. Keep credentials out of this form." })));
