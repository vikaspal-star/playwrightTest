...
- Project PDF/DOCX/TXT/Markdown uploads now create reviewed requirements and linked test designs; see [the document workflow](DOCUMENT_REQUIREMENTS.md). Feeding documents directly into conversational-agent knowledge remains unsupported, as do Jira/Confluence synchronization, Postman import, multi-phase login/session-setup endpoints, WebSocket/SSE target transports and automatic extraction of server-created session tokens. A custom adapter is needed if the target does not accept the configured POST template and client session/history.
